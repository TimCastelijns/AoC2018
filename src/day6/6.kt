...
private val input = File("src/day6/input").readLines()
private val chars = (IntRange(65, 90) + IntRange(97, 122)).map { it.toChar() }
...
private val x = coordinates.sortedByDescending { it.x }[0].x + 1
private val y = coordinates.sortedByDescending { it.y }[0].y + 1

// Calculate which x/y define the boundaries.
private val top = coordinates.minBy { it.y }!!.y
private val bottom = coordinates.maxBy { it.y }!!.y
private val left = coordinates.minBy { it.x }!!.x
private val right = coordinates.maxBy { it.x }!!.x
    ...
    for (i in 0 until x) {
        for (j in 0 until y) {
            ...
            if (manhattanDistanceSum < 10000) {
                grid[location.x][location.y] = location.name
            }
        }
    }
...
private data class Coordinate(val name: Char, val x: Int, val y: Int)
